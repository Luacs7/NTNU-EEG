function no_shuffle_data_train_test(EEG,file,EEG_void)
prop_train=0.5;
...
t_marker=7;%define the time between each marker in seconds
t_post_marker = 0;
...
NEWEEG_train.data=zeros(8,N_tot_train);
...
NEWEEG_test.data=zeros(8,N_tot_test);
...
for j= 1:N_events_train
    ...
    if  strcmp(event_.type,'right') || strcmp(event_.type,'left')
        ...
        if event_.latency <=  N_points_post 
            ...
            NEWEEG_train.data(:,Point_ind:Point_ind + N_missing-1) = zeros(8,N_missing)
        end
        if event_.latency+N_points -1 > EEG.pnts
            N_missing_end = event_.latency+N_points -1  -  EEG.pnts
            NEWEEG_train.data(:,Point_ind + N_points + N_points_post -N_missing_end :Point_ind + N_points + N_points_post -1) = zeros(8,N_missing_end)
        end
...
for j= 1:N_events_test
    ...
    if  strcmp(event_.type,'right') || strcmp(event_.type,'left')
        ...
        if event_.latency <=  N_points_post 
            N_missing = N_points_post- event_.latency+1
            NEWEEG_test.data(:,Point_ind:Point_ind + N_missing-1) = zeros(8,N_missing)
        end
        if event_.latency+N_points -1 > EEG.pnts
            N_missing_end = event_.latency+N_points -1  -  EEG.pnts
            NEWEEG_test.data(:,Point_ind + N_points + N_points_post -N_missing_end :Point_ind + N_points + N_points_post -1) = zeros(8,N_missing_end)
...
pop_saveset( NEWEEG_train, 'train_data', file);
NEWEEG_test.pnts=length(NEWEEG_test.data);
...
pop_saveset( NEWEEG_test, 'test_data', file);
